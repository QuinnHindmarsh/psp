...
while play != 'n': #loops until player quits
    ...
    if play == 'n':#prints stats
        ...
        while rounds != 0 and end != True:#loops until all rounds have been played or a charather is dead
            ...
            player_roll = []  # resets players hand
            for itteration in range(0, 5, 1): #rolls 5 dice for player
                player_roll.append(random.randint(1, 6))
            ...
            for item in player_roll:
                roll_frequency[item] += 1

            # finds the appropraite damage multiplier to use based on the fequencies of certain numbers in the roll
            damage_multiplier = 1
            ...
            player_damage = damage_multiplier * sum_damage#damage player inflicts
            ...
            """                                                     ***DRAGONS TURN***                                                     """
            input()#pauses untill user enters any key - makes it easier to follow game
            dragon_roll = [] #resets dragon hand
            for itteration in range(0, 5, 1):#rolls 5 dice for dragon
                dragon_roll.append(random.randint(1, 6))

            print('Dragon rolled: ')
            display_dice(dragon_roll)

            roll_frequency = [0, 0, 0, 0, 0, 0,0]  # all elements but element0 corripsond to the amount of times that number was rolled. eg if roll_frequency[2] == 3 then 3 2s where rolled
            for item in dragon_roll:
                roll_frequency[item] += 1
            ...
            sum_damage = 0#holds the sum of the dice rolled before multipliers
            for i in dragon_roll:
                sum_damage += i

            dragon_damage = damage_multiplier * sum_damage
